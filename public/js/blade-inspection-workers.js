(function () {
  ...
  const config = window.BIR_CONFIG || { technicians: [] };
  ...
  function reindex() {
    container.querySelectorAll('.technician-row').forEach((row, index) => {
      ...
      label.textContent =
        index === 0
          ? 'Coastal Composite Technician *'
          : `Coastal Composite Technician ${index + 1}`;
  ...
  addBtn.addEventListener('click', () => {
    ...
    row.innerHTML = `
      <div class="form-group">
        ...
        <select name="technician_${index}" class="form-control">
          ${optionHtml(config.technicians)}
        </select>
      </div>
    ...
    if (window.SearchableSelect) {
      SearchableSelect.init(row.querySelector('select'));
    }
  });
})();